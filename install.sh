...
# IPv6 DNS answers even though IPv6 routing is unavailable, which makes ACME API
# calls fail intermittently with curl error 35. In auto mode, keep dual-stack
# when IPv6 works and only force IPv4 when IPv6 is broken.
prepare_acme_curl_config() {
    local force_ipv4="${ACME_FORCE_IPV4:-auto}"
    local acme_uri="${ACME_CA_URI:-https://acme-v02.api.letsencrypt.org/directory}"

    if [ "$force_ipv4" = "auto" ]; then
        if timeout 10 curl -6fsS --connect-timeout 5 "$acme_uri" >/dev/null 2>&1; then
            force_ipv4=0
            log_success "IPv6 ACME connectivity works; leaving ACME curl dual-stack"
        else
            force_ipv4=1
            log_warning "IPv6 ACME connectivity failed; forcing ACME curl over IPv4"
        fi
    fi

    if [ "$force_ipv4" = "0" ]; then
        cat > acme-curlrc <<'EOF'
# Generated by install.sh.
# ACME_FORCE_IPV4=0/auto-with-working-IPv6: do not force curl address family.
EOF
        return 0
    fi
    ...
    cat > acme-curlrc <<'EOF'
# Generated by install.sh.
# Force acme.sh's outbound CA API requests through IPv4.
...
DEBUG=0
ACME_FORCE_IPV4=${ACME_FORCE_IPV4:-auto}
